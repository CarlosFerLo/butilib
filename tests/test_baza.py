...
def test_baza_has_initial_player_played_cards_attributes():
    baza = butilib.Baza(
        initial_player=0, cards=[butilib.Card(suit=butilib.OROS, number=1)]
    )
...
def test_baza_initial_player_is_a_int_between_0_and_3():
    pytest.raises(
        ...
        initial_player=-1,
        cards=[butilib.Card(suit=butilib.OROS, number=1)],
    )
    pytest.raises(
        pydantic.ValidationError,
        butilib.Baza,
...
def test_baza_cards_has_no_repeated_elements():
    pytest.raises(
        ...
        initial_player=0,
        cards=[
            butilib.Card(suit=butilib.OROS, number=1),
            butilib.Card(suit=butilib.OROS, number=1),
        ],
    )
...
def test_history_bazas_has_a_number_of_bazas_between_0_to_12():
    ...
    for s in butilib.Suit:
        b = [
            butilib.Baza(
                initial_player=0,
                cards=[butilib.Card(number=i, suit=s) for i in [9, 1, 12, 11]],
            ),
            butilib.Baza(
                initial_player=0,
                cards=[butilib.Card(number=i, suit=s) for i in [10, 8, 7, 6]],
            ),
            butilib.Baza(
                initial_player=0,
                cards=[butilib.Card(number=i, suit=s) for i in [5, 4, 3, 2]],
            ),
        ]
        bazas.extend(b)
    ...
    bazas.append(
        butilib.Baza(
            initial_player=0,
            cards=[butilib.Card(number=i, suit=butilib.COPAS) for i in [9, 2, 3, 4]],
        )
    )
...
def test_history_checks_that_no_cards_are_repeated_on_the_bazas():
    pytest.raises(
        ...
        bazas=[
            ...
            butilib.Baza(
                initial_player=0,
                cards=[butilib.Card(number=i, suit=butilib.OROS) for i in [2, 5, 6, 7]],
            ),
        ],
    )
