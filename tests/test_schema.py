...
def test_cantar_input_has_cards_and_delegated_fields_and_expects_exactly_twelve_cards():
    ...
    cantar_input = butilib.CantarInput(
        cards=butilib.CardSet(cards=card_list), delegated=False
    )
    ...
    pytest.raises(
        ...
        butilib.CantarInput,
        cards=butilib.CardSet(cards=card_list),
        delegated=True,
    )
...
def test_cantar_output_has_optional_suit_field_a_butifarra_field_and_delegate_field_that_can_not_be_two_set_to_a_positive_state():
    ...
    pytest.raises(
        pydantic.ValidationError,
        butilib.CantarOutput,
        suit=butilib.OROS,
        delegate=True,
        butifarra=True,
    )
    pytest.raises(
        pydantic.ValidationError, butilib.CantarOutput, suit=butilib.OROS, delegate=True
    )
    pytest.raises(
        pydantic.ValidationError,
        butilib.CantarOutput,
    ...
    pytest.raises(
        pydantic.ValidationError, butilib.CantarOutput, delegate=True, butifarra=True
    )
    ...
    history = butilib.History(
        bazas=[
            butilib.Baza(
                initial_player=0,
                cards=[
                    butilib.Card(number=i, suit=butilib.OROS) for i in [8, 10, 12, 9]
                ],
            ),
            butilib.Baza(
                initial_player=3,
                cards=[
                    butilib.Card(number=i, suit=butilib.ESPADAS) for i in [9, 2, 3, 5]
                ],
            ),
        ]
    )
...
def test_play_output_has_card_and_forced_attributes_with_False_as_default_value_for_forced():
    ...
    play_output = butilib.PlayOutput(
    ...
    play_input = butilib.PlayInput(
        history=butilib.History(
            bazas=[
                butilib.Baza(
                    cards=[
                        butilib.Card(number=i, suit=butilib.BASTOS)
                        for i in [8, 10, 12, 9]
                    ],
                    initial_player=2,
                )
