...
class CantarOutput(BaseModel):
    ...
    def check_only_one_option_was_set(self):
        if self.suit is not None and self.delegate and self.butifarra:
            raise ValueError(
                "Only one of suit, butifarra and delegate fields can be set to non None/False values."
            )
        if self.suit is not None and self.delegate:
            raise ValueError(
                "Not both suit and delegate fields can be set to non None/False values."
            )
        if self.suit is not None and self.butifarra:
            raise ValueError(
                "Not both suit and butifarra fields can be set to non None/False values."
            )
        if self.butifarra and self.delegate:
            raise ValueError(
                "Not both delegate and butifarra fields can be set to non None/False values."
            )
        if self.suit is None and not self.delegate and not self.butifarra:
            raise ValueError(
                "Must set one of the suit, delegate or butifarra fields to non None/False values."
            )
...
class ContrarInput(BaseModel):
    ...
    def check_it_is_possible_to_be_in_that_situation(self):
        if self.contrada == SANT_VICENTADA:
            raise ValueError(
                "Once you have called Sant Vicenç you can no longer continue to contrar."
            )
    ...
    def check_only_one_of_triumph_and_butifarra_attributes_is_not_none(self):
        if self.butifarra is False and self.triumph is None:
            raise ValueError(
                "Only one of suit or butifarra fields can be set to non None/False values."
            )
        if self.butifarra is True and self.triumph is not None:
            raise ValueError(
                "Must set one of the suit or butifarra fields to non None/False values."
            )
...
class PlayInput(BaseModel):
    ...
    def check_history_is_consistent(self):
        ...
        if (
            ...
            and len(self.cards) != (self.player_number - prev_win) % 4
        ):
            raise ValueError(
                "The player_number attribute is not consistent with the cards and history attributes."
            )

        return self

    @model_validator(mode="after")
    def check_number_of_cards_in_card_set_is_consistent_with_the_number_of_bazas_in_history(
        self,
    ):
        if len(self.card_set) != 12 - len(self.history):
            raise ValueError(
                f"The number of cards on the card set is not consistent with the number of bazas in the history. Number of cards: {len(self.card_set)}. Number of bazas: {len(self.history)}."
            )
        return self
